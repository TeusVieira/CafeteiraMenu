...
public class funcionarioDaoJDBC implements FuncionarioDao {
    ...
    public void cadastrarFuncionario(funcionario f) {
        ...
        try {
            st = conn.prepareStatement("insert into funcionario(nome, sexo, email, telefone) values(?,?,?,?)");
    ...
    public funcionario procurarFuncionario(int id) {
        ...
        try {
            st = conn.prepareStatement("Select id, nome from funcionario where id_funcionario = ?");
            st.setInt(1, id);
            rs = st.executeQuery();
            if (rs.next()){
